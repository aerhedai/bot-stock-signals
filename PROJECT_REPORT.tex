...
This early structure reflected a deliberate decision to treat repository hygiene as a first-class concern from the start. Environment variables were externalised before a single line of application code was committed, avoiding the common pattern of credentials hardcoded in early prototypes and later needing to be scrubbed from history.

The initial architecture diagram committed at this stage already showed the three-service model (backend, frontend, Telegram bot) that the project would ultimately deliver, suggesting that the high-level design was largely settled before implementation began. The key open question was which analysis engines would be implemented and how they would connect.
...
The initial frontend was functional but unstyled. Between 20 January and 3 March 2026, a sequence of nine frontend commits progressively built a coherent design system and applied it across all pages. This phase represents a significant investment and reflects a deliberate decision: a signal platform that looks untrustworthy will not be used, regardless of the quality of the analysis underneath.

The design system was built first (20--24 January): CSS custom properties defined a dark theme with surface-level colour tokens (\texttt{--surface-root}, \texttt{--surface-card}, \texttt{--surface-hover}), a text hierarchy (\texttt{--text-primary}, \texttt{--text-secondary}, \texttt{--text-muted}), and semantic colours for success, warning, and error states. An icon library was developed alongside. Building the tokens first meant that subsequent component restyling could apply a consistent language rather than hardcoding colours per component.

The sidebar was redesigned next (28--31 January): sectioned navigation with icons and a health status indicator replaced the original list. Signal cards and the news card were then restyled with the design tokens (3--6 February). A page header component and a unified predictions routing structure followed (10--13 February), relocating the stock and crypto pages under a shared \texttt{/predictions} hierarchy. The dashboard itself was redesigned (17--20 February) with summary cards, service status indicators, and preview sections for recent signals and news.
